        ...
        <div className="absolute flex flex-col items-left transition-all duration-[1000ms] pb-[5px] max-w-[313px] max-h-[503px] border-[1px] border-[#E5E7EA]  bg-[#FFFFFF] rounded-md shadow-lg">
          ...
          <div className='flex flex-col gap-[10px]'>
            ...
            <div>
              ...
              {
                filterData.map((ct:string, index:number ) => (
                  <div key={index} className='flex justify-between px-3 mt-[3px]'>
                    <input  id={`search-${index}`} type="checkbox" checked = {isChecked[index]} onChange={() => handleCheckboxChange(index)}  className="peer test cursor-pointer appearance-none mt-2 ml-[17rem]  h-[15px] w-[15px] border border-[#E5E7EA] rounded-full bg-white checked:bg-[#22385F] checked:border-transparent focus:outline-none" />
                    <label htmlFor={`search-${index}`}  className={`text-[#6D6D6D] absolute font-noto font-normal  mr-[7rem] text-[14px] leading-[19.07px] peer-checked:text-[#22385F] `}>{ct}</label>
                  </div> 
                ))}
